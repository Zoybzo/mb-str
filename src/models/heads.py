...
class CGCDotProductPredictionHead(nn.Module):
    ...
    def __init__(self, d_model, n_b, n_e_sh, n_e_sp, num_items, token_embeddings):
        ...
        self.shared_experts = nn.ModuleList(
            [nn.Sequential(nn.Linear(d_model, d_model)) for i in range(self.n_e_sh)]
        )
        self.specific_experts = nn.ModuleList(
            [
                nn.Sequential(nn.Linear(d_model, d_model))
                for i in range(self.n_b * self.n_e_sp)
            ]
        )
        ...
        self.w_gates = nn.Parameter(
            torch.randn(self.n_b, d_model, self.n_e_sh + self.n_e_sp),
            requires_grad=True,
        )
        self.token_embeddings = token_embeddings
    ...
    def mmoe_process(self, x, b_seq):
        ...
        experts_o_tensor = torch.stack(
            [
                torch.stack(
                    shared_experts_o
                    + specific_experts_o[i * self.n_e_sp : (i + 1) * self.n_e_sp]
                )
